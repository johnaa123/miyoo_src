...
{
  const InputDialog::Button b = { label:
                                  label, category:
                                  category, defaultKey:
                                  defaultKey,
                                  defaultAltKey:
...
InputDialog* GambatteSource::createInputDialog()
{
  ...
  struct FastForwardAct : InputDialog::Button::Action
  {
    ...
    virtual void buttonPressed()
    {
      source->emitSetTurbo(true);
    }
    virtual void buttonReleased()
    {
      source->emitSetTurbo(false);
    }
  };
...
const std::vector<VideoDialog::VideoSourceInfo> GambatteSource::generateVideoSourceInfos()
{
  ...
  {
    const VideoDialog::VideoSourceInfo vsi = { label:
                                               VfilterInfo::get(i).handle,
                                               width:
                                               VfilterInfo::get(i).outWidth, height:
                                               VfilterInfo::get(i).outHeight
                                             };
