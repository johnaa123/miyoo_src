...
public:
  ...
  QRgb getColor(unsigned index) const
  {
    return picker[index & 3]->getColor();
  }
  void setColor(unsigned index, QRgb color)
  {
    picker[index & 3]->setColor(color);
  }
...
public:
  ...
  ~PaletteDialog();
  QRgb getColor(unsigned palnum, unsigned colornum) const
  {
    return currentColors[palnum < 3 ? palnum : 2][colornum & 3];
  }
  void externalChange();
