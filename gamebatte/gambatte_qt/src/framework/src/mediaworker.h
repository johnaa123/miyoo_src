...
private:
  ...
  class MeanQueue
  {
    ...
    struct Elem
    {
      ...
      Elem() {} Elem(long sp, long dp) : sumpart(sp), dsumpart(dp) {}
    };
...
public:
  ...
  {
    return sampleBuffer.source();
  }
  SyncVar& waitingForSync() /*const */
  {
    return waitingForSync_;
  }
  void start();
  void stop();
  void pause();
  void unpause()
  {
    pauseVar.unpause(PauseVar::PAUSE_BIT);
  }
  void qPause()
  {
    pauseVar.pause(PauseVar::QPAUSE_BIT);
  }
  void qUnpause()
  {
    pauseVar.unpause(PauseVar::QPAUSE_BIT);
  }
  void recover()
  {
    pauseVar.unpause(PauseVar::FAIL_BIT);
  }
  bool paused() const
  {
    return pauseVar.waitingForUnpause();
  }
  ...
  void setFrameTimeEstimate(const long ftest)
  {
    AtomicVar<long>::Locked(frameTimeEst).set(ftest);
  }
  bool frameStep();

  void setFastForwardSpeed(unsigned speed)
  {
    turboSkip.setSpeed(speed);
  }
  unsigned fastForwardSpeed() const
  {
    return turboSkip.speed();
  }
  void setFastForward(bool enable);
  bool fastForward() const
  {
    return turboSkip.isEnabled();
  }

  void updateJoysticks();

  template<class T>
  void pushCall(const T &t)
  {
    pauseVar.pushCall(t, AtomicVar<bool>::ConstLocked(doneVar).get());
  }
};
