...
class MediaWidget : public QObject
{
  ...
  public:
    ...
    void dec(unsigned dec, MediaWidget &mw)
    {
      modifyPaused(paused - dec, mw);
    }
    void inc(unsigned inc, MediaWidget &mw)
    {
      modifyPaused(paused + inc, mw);
    }
    void set(unsigned bm, MediaWidget &mw)
    {
      modifyPaused(paused | bm, mw);
    }
    void unset(unsigned bm, MediaWidget &mw)
    {
      modifyPaused(paused & ~bm, mw);
    }
  };
  ...
  void emitVideoBlitterFailure()
  {
    emit videoBlitterFailure();
  }
...
public:
  ...
  {
    return blitterContainer;
  }

  /** @return compositionChange */
  bool winEvent(const void *message)
  {
    return dwmControl_.winEvent(message);
  }
  void hideEvent()
  {
    dwmControl_.hideEvent();
  }
  void showEvent(const QWidget *parent)
  {
    dwmControl_.showEvent();
  ...
  void moveEvent(const QWidget *parent)
  {
    fullModeToggler->setScreen(parent);
  }
  ...
  {
    return blitterContainer->sourceSize();
  }
  ...
  {
    return BlitterConf(blitters[blitterNo]);
  }
  const ConstBlitterConf blitterConf(std::size_t blitterNo) const
  {
    return ConstBlitterConf(blitters[blitterNo]);
  }
  std::size_t numBlitters() const
  {
    return blitters.size();
  }
  ...
  {
    worker->setFastForwardSpeed(speed);
  }
  ...
  {
    return fullModeToggler->modeVector(screen);
  }
  ...
  {
    return fullModeToggler->screens();
  }
  std::size_t currentResIndex(std::size_t screen) const
  {
    return fullModeToggler->currentResIndex(screen);
  }
  std::size_t currentRateIndex(std::size_t screen) const
  {
    return fullModeToggler->currentRateIndex(screen);
  }
  std::size_t currentScreen() const
  {
    return fullModeToggler->screen();
  }
  const QRect fullScreenRect(const QWidget *w) const
  {
    return fullModeToggler->fullScreenRect(w);
  }
  void setFullMode(bool fullscreen)
  {
    fullModeToggler->setFullMode(fullscreen);
  }
  bool isFullMode() const
  {
    return fullModeToggler->isFullMode();
  }
  void parentExclusiveEvent(bool exclusive)
  {
    blitterContainer->parentExclusiveEvent(exclusive);
  }

  const AudioEngineConf audioEngineConf(std::size_t aeNo)
  {
    return AudioEngineConf(audioEngines[aeNo]);
  }
  const ConstAudioEngineConf audioEngineConf(std::size_t aeNo) const
  {
    return ConstAudioEngineConf(audioEngines[aeNo]);
  }
  std::size_t numAudioEngines() const
  {
    return audioEngines.size();
  }
  ...
  {
    return ResamplerInfo::num();
  }
  const char* resamplerDesc(std::size_t resamplerNo) const
  {
    return ResamplerInfo::get(resamplerNo).desc;
  }
  ...
  {
    dwmControl_.setDwmTripleBuffer(enable);
  }
  void setFastForward(bool enable)
  {
    worker->setFastForward(enable);
  }
  void setSyncToRefreshRate(bool on)
  {
    frameRateControl.setRefreshRateSync(on);
  }
