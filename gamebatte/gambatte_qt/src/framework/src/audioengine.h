...
public:
  ...
  virtual ~AudioEngine() {}
  int init(int rate, unsigned msLatency)
  {
    mut.lock();
    rate_ = doInit(rate, msLatency);
    mut.unlock();
    return rate_;
  }
  virtual void uninit() {}
  ...
  virtual const BufferState bufferState() const
  {
    static const BufferState s = { BufferState::NOT_SUPPORTED, BufferState::NOT_SUPPORTED };
    return s;
  }
  virtual void pause() {}

  /** @return success */
  virtual bool flushPausedBuffers() const
  {
    return false;
  }
