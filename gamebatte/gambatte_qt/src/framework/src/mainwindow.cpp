...
void MainWindow::setVideoBlitter(std::size_t blitterNo)
{
  w_->setVideoBlitter(blitterNo);
}
void MainWindow::setAspectRatio(const QSize &ar)
{
  w_->setAspectRatio(ar);
}
void MainWindow::setScalingMethod(ScalingMethod smet)
{
  w_->setScalingMethod(smet);
}

void MainWindow::setAudioOut(std::size_t engineNo, unsigned srateHz, unsigned msecLatency)
{
  w_->setAudioOut(engineNo, srateHz, msecLatency);
}
void MainWindow::setFrameTime(long num, long denom)
{
  w_->setFrameTime(num, denom);
}
...
{
  w_->setSamplesPerFrame(num, denom);
}
void MainWindow::setSyncToRefreshRate(bool on)
{
  w_->setSyncToRefreshRate(on);
}
...
const std::vector<ResInfo>& MainWindow::modeVector(std::size_t screen) const
{
  return w_->modeVector(screen);
}
const QString MainWindow::screenName(std::size_t screen) const
{
  return w_->screenName(screen);
}
std::size_t MainWindow::screens() const
{
  return w_->screens();
}
std::size_t MainWindow::currentResIndex(std::size_t screen) const
{
  return w_->currentResIndex(screen);
}
std::size_t MainWindow::currentRateIndex(std::size_t screen) const
{
  return w_->currentRateIndex(screen);
}
std::size_t MainWindow::currentScreen() const
{
  return w_->currentScreen();
}
const BlitterConf MainWindow::currentBlitterConf()
{
  return w_->currentBlitterConf();
}
...
void MainWindow::setFastForwardSpeed(unsigned speed)
{
  w_->setFastForwardSpeed(speed);
}
void MainWindow::setFastForward(bool enable)
{
  w_->setFastForward(enable);
}
void MainWindow::pause(unsigned bitmask)
{
  w_->pause(bitmask);
}
void MainWindow::unpause(unsigned bitmask)
{
  w_->unpause(bitmask);
}
void MainWindow::incPause(unsigned inc)
{
  w_->incPause(inc);
}
void MainWindow::decPause(unsigned dec)
{
  w_->decPause(dec);
}
...
const BlitterConf MainWindow::blitterConf(std::size_t blitterNo)
{
  return w_->blitterConf(blitterNo);
}
const ConstBlitterConf MainWindow::blitterConf(std::size_t blitterNo) const
{
  return w_->blitterConf(blitterNo);
}
std::size_t MainWindow::numBlitters() const
{
  return w_->numBlitters();
}
const AudioEngineConf MainWindow::audioEngineConf(std::size_t aeNo)
{
  return w_->audioEngineConf(aeNo);
}
const ConstAudioEngineConf MainWindow::audioEngineConf(std::size_t aeNo) const
{
  return w_->audioEngineConf(aeNo);
}
std::size_t MainWindow::numAudioEngines() const
{
  return w_->numAudioEngines();
}
std::size_t MainWindow::numResamplers() const
{
  return w_->numResamplers();
}
const char* MainWindow::resamplerDesc(std::size_t resamplerNo) const
{
  return w_->resamplerDesc(resamplerNo);
}
void MainWindow::setResampler(std::size_t resamplerNo)
{
  w_->setResampler(resamplerNo);
}
void MainWindow::resetAudio()
{
  w_->resetAudio();
}
void MainWindow::setDwmTripleBuffer(bool enable)
{
  w_->setDwmTripleBuffer(enable);
}
bool MainWindow::hasDwmCapability()
{
  return DwmControl::hasDwmCapability();
}
bool MainWindow::isDwmCompositionEnabled()
{
  return DwmControl::isCompositingEnabled();
}
...
void MainWindow::setPauseOnFocusOut(unsigned bitmask)
{
  w_->setPauseOnFocusOut(bitmask, hasFocus());
}
