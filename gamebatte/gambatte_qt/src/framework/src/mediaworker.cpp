...
MediaWorker::MeanQueue::MeanQueue(const long mean, const long var) : q(sz, Elem(mean, var))
{
  sum = mean * sz;
  dsum = var * sz;
}
...
public:
  ...
  bool flushPausedBuffers() const
  {
    return ae_.flushPausedBuffers();
  }
  long rate() const
  {
    return ae_.rate() > 0 ? ae_.rate() : rate_;
  }
  long estimatedRate() const
  {
    return estrate_;
  }
...
{
  MediaWorker::TurboSkip &ts;
  const bool enable;
  void operator()()
  {
    ts.setEnabled(enable);
  }
};
