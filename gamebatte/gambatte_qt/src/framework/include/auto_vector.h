...
public:
  ...
  auto_vector(auto_vector &v) : std::vector<T*, Allocator>()
  {
    swap(v);
  }
  ...
  auto_vector& operator=(auto_vector &v)
  {
    clear();
    swap(v);
  }
  ...
  {
    std::vector<T*, Allocator>::resize(sz, 0);
  }
  ...
  {
    std::vector<T*, Allocator>::insert(position, x);
  }
  void insert(iterator position, size_type n)
  {
    std::vector<T*, Allocator>::insert(position, n, 0);
  }
  ...
  const std::vector<T*,Allocator> get() const
  {
    return *this;
  }

  operator const auto_vector_temporary()
  {
    std::vector<T*, Allocator> v;
    v.swap(*this);
    return auto_vector_temporary(v);
  }
};
