...
int init_menu()
{
  ...
  RWops = SDL_RWFromMem(menu_bk_img, sizeof(menu_bk_img));
  menu_bk = IMG_LoadPNG_RW(RWops);
  SDL_FreeRW(RWops);
  if (!menu_bk)
  {
    fprintf(stderr, "menu: background load error\n");
    exit(1);
  }
