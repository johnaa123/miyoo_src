...
public:
  ...
  {
    return "\t\tStart in full screen mode\n";
  }
  bool startFull() const
  {
    return full;
  }
};
...
public:
  ...
  const char* getDesc() const
  {
    return "\tSupport certain multicart ROM images by not strictly respecting ROM header MBC type\n";
  }
  bool enabled() const
  {
    return enabled_;
...
public:
  ...
  const char* getDesc() const
  {
    return "\t\tUse YUV overlay for (usually faster) scaling\n";
  }
  bool useYuv() const
  {
    return yuv;
...
public:
  ...
  unsigned resamplerNumber() const
  {
    return resamplerNo;
  }
};
...
InputOption::InputOption() : DescOption("input", 'i', 8)
{
  ...
  keys[2].key  = SDLK_LALT;
  keys[3].key  = SDLK_LCTRL;
  keys[4].key  = SDLK_UP;
      ...
      else
      {
        ...
        switch (type)
        {
          case 'a':
            ...
            {
              case '+':
                id.jdata.dir = JoyData::UP;
                break;
              case '-':
                id.jdata.dir = JoyData::DOWN;
                break;
              default:
                continue;
            }
          ...
          case 'h':
            switch (dir)
            {
              ...
              case 'd':
                id.jdata.dir = JoyData::DOWN;
              case 'l':
                id.jdata.dir = JoyData::LEFT;
              case 'r':
                id.jdata.dir = JoyData::RIGHT;
              default:
                continue;
            }
...

  {
    ...
    for (unsigned i = 0; i < 8; ++i)
    {
      ...
      else
      {
        ...
        switch (id.type)
        {
          case InputOption::InputId::JOYBUT:
            jbMap.insert(pair);
            break;
          case InputOption::InputId::JOYAX:
            jaMap.insert(pair);
            break;
          case InputOption::InputId::JOYHAT:
            jhMap.insert(pair);
            break;
          default:
            break;
