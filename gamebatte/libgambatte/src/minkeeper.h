...
{
template<int n> struct CeiledLog2
{
  enum { RESULT = 1 + CeiledLog2<(n + 1) / 2>::RESULT };
};
template<> struct CeiledLog2<1>
{
  enum { RESULT = 0 };
};

template<int v, int n> struct RoundedDiv2n
{
  enum { RESULT = RoundedDiv2n<(v + 1) / 2, n - 1>::RESULT };
};
template<int v> struct RoundedDiv2n<v,1>
{
  enum { RESULT = v };
};

template<template<int> class T, int n> struct Sum
{
  enum { RESULT = T<n-1>::RESULT + Sum<T, n-1>::RESULT };
};
template<template<int> class T> struct Sum<T,0>
{
  enum { RESULT = 0 };
};
}
...
class MinKeeper
{
  ...
  template<int l> struct Num
  {
    enum { RESULT = MinKeeperUtil::RoundedDiv2n<ids, LEVELS + 1 - l>::RESULT };
  };
  template<int l> struct Sum
  {
    enum { RESULT = MinKeeperUtil::Sum<Num, l>::RESULT };
  };
...
public:
  ...
  unsigned long value(const int id) const
  {
    return values[id];
  }
};
