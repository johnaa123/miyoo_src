...
// 	unsigned interrupt(unsigned address, unsigned cycleCounter);
  ...
  {
    return memory.romTitle();
  }

  void setSoundBuffer(uint_least32_t *const buf)
  {
    memory.setSoundBuffer(buf);
  }
  unsigned fillSoundBuffer()
  {
    return memory.fillSoundBuffer(cycleCounter_);
  }
  ...
  {
    memory.setGameGenie(codes);
  }
  void setGameShark(const std::string &codes)
  {
    memory.setGameShark(codes);
  }
};
