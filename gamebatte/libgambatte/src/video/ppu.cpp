...
static inline unsigned weMasterCheckPriorToLyIncLineCycle(const bool cgb)
{
  return 450 - cgb;
}
static inline unsigned weMasterCheckAfterLyIncLineCycle(const bool cgb)
{
  return 454 - cgb;
}
static inline unsigned m3StartLineCycle(const bool /*cgb*/)
{
  return 83;
}
      ...
      else
      {
        ...
        do
        {
          ...
          if (!((attrib | sattrib) & bgenmask))
          {
            ...
            {
              case 8:
                if ((spword >> 14    ) && id < idtab[pos+7])
                {
                  idtab[pos+7] = id;
                  dst[pos+7] = spPalette[spword >> 14    ];
                }
              case 7:
                if ((spword >> 12 & 3) && id < idtab[pos+6])
                {
                  idtab[pos+6] = id;
                  dst[pos+6] = spPalette[spword >> 12 & 3];
                }
              case 6:
                if ((spword >> 10 & 3) && id < idtab[pos+5])
                {
                  idtab[pos+5] = id;
                  dst[pos+5] = spPalette[spword >> 10 & 3];
                }
              case 5:
                if ((spword >>  8 & 3) && id < idtab[pos+4])
                {
                  idtab[pos+4] = id;
                  dst[pos+4] = spPalette[spword >>  8 & 3];
                }
              case 4:
                if ((spword >>  6 & 3) && id < idtab[pos+3])
                {
                  idtab[pos+3] = id;
                  dst[pos+3] = spPalette[spword >>  6 & 3];
                }
              case 3:
                if ((spword >>  4 & 3) && id < idtab[pos+2])
                {
                  idtab[pos+2] = id;
                  dst[pos+2] = spPalette[spword >>  4 & 3];
                }
              case 2:
                if ((spword >>  2 & 3) && id < idtab[pos+1])
                {
                  idtab[pos+1] = id;
                  dst[pos+1] = spPalette[spword >>  2 & 3];
                }
...
static const PPUState * decodeM3LoopState(const unsigned state)
{
  switch (state)
  {
    case M3Loop::Tile::ID0:
      return &M3Loop::Tile::f0_;
    case M3Loop::Tile::ID1:
      return &M3Loop::Tile::f1_;
    case M3Loop::Tile::ID2:
      return &M3Loop::Tile::f2_;
    case M3Loop::Tile::ID3:
      return &M3Loop::Tile::f3_;
    case M3Loop::Tile::ID4:
      return &M3Loop::Tile::f4_;
    case M3Loop::Tile::ID5:
      return &M3Loop::Tile::f5_;

    case M3Loop::LoadSprites::ID0:
      return &M3Loop::LoadSprites::f0_;
    case M3Loop::LoadSprites::ID1:
      return &M3Loop::LoadSprites::f1_;
    case M3Loop::LoadSprites::ID2:
      return &M3Loop::LoadSprites::f2_;
    case M3Loop::LoadSprites::ID3:
      return &M3Loop::LoadSprites::f3_;
    case M3Loop::LoadSprites::ID4:
      return &M3Loop::LoadSprites::f4_;
    case M3Loop::LoadSprites::ID5:
      return &M3Loop::LoadSprites::f5_;

    case M3Loop::StartWindowDraw::ID0:
      return &M3Loop::StartWindowDraw::f0_;
    case M3Loop::StartWindowDraw::ID1:
      return &M3Loop::StartWindowDraw::f1_;
    case M3Loop::StartWindowDraw::ID2:
      return &M3Loop::StartWindowDraw::f2_;
    case M3Loop::StartWindowDraw::ID3:
      return &M3Loop::StartWindowDraw::f3_;
    case M3Loop::StartWindowDraw::ID4:
      return &M3Loop::StartWindowDraw::f4_;
    case M3Loop::StartWindowDraw::ID5:
      return &M3Loop::StartWindowDraw::f5_;
  }
