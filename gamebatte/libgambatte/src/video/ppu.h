...
class PPUFrameBuf
{
  ...
  static uint_least32_t * nullfbline()
  {
    static uint_least32_t nullfbline_[160];
    return nullfbline_;
  }

public:
  PPUFrameBuf() : buf_(0), fbline_(nullfbline()), pitch_(0) {}
  uint_least32_t * fb() const
  {
    return buf_;
  }
  uint_least32_t * fbline() const
  {
    return fbline_;
  }
  int pitch() const
  ...
  void setBuf(uint_least32_t *const buf, const int pitch)
  {
    buf_ = buf;
    pitch_ = pitch;
    fbline_ = nullfbline();
  }
  void setFbline(const unsigned ly)
  {
    fbline_ = buf_ ? buf_ + static_cast<long>(ly) * static_cast<long>(pitch_) : nullfbline();
  }
};
...
public:
  ...
  void doLyCountEvent()
  {
    p_.lyCounter.doEvent();
  }
  unsigned long doSpriteMapEvent(unsigned long time)
  {
    return p_.spriteMapper.doEvent(time);
  }
  const PPUFrameBuf & frameBuf() const
  {
    return p_.framebuf;
  }
  ...
  unsigned long lastM0Time() const
  {
    return p_.lastM0Time;
  }
  unsigned lcdc() const
  {
    return p_.lcdc;
  ...
  const LyCounter & lyCounter() const
  {
    return p_.lyCounter;
  }
  ...
  void oamChange(unsigned long cc)
  {
    p_.spriteMapper.oamChange(cc);
  }
  void oamChange(const unsigned char *oamram, unsigned long cc)
  {
    p_.spriteMapper.oamChange(oamram, cc);
  }
  ...
  void setFrameBuf(uint_least32_t *buf, unsigned pitch)
  {
    p_.framebuf.setBuf(buf, pitch);
  }
  void setLcdc(unsigned lcdc, unsigned long cc);
  void setScx(const unsigned scx)
  {
    p_.scx = scx;
  }
  void setScy(const unsigned scy)
  {
    p_.scy = scy;
  }
  void setStatePtrs(SaveState &ss)
  {
    p_.spriteMapper.setStatePtrs(ss);
  }
  void setWx(const unsigned wx)
  {
    p_.wx = wx;
  }
  void setWy(const unsigned wy)
  {
    p_.wy = wy;
  }
  void updateWy2()
