...
void GraphicsMode ( void )
{
    ...
    rgb_surface = SDL_SetVideoMode (320, 240, 16, flags);
	  if (rgb_surface == NULL){
		  Error ("Could not set video mode\n");
	  }

   sdl_surface = SDL_CreateRGBSurface(SDL_SWSURFACE, iGLOBAL_SCREENWIDTH, iGLOBAL_SCREENHEIGHT, 8, 0, 0, 0, 0);
	if (sdl_surface == NULL)
	{
...
void rgb_blit()
{
	...
	unsigned short *dest_pixel = rgb_surface->pixels+(40*320), *prev_line = dest_pixel;
