...
int run_game(int game)
{
  ...
  // copy configuration
  if (options.color_depth == 16 || (options.color_depth != 8 && (Machine->gamedrv->flags & GAME_REQUIRES_16BIT))){
...
// Initialize the emulated machine (load the roms, initialize the various subsystems...). 
...
// run the emulation. 
// start the various subsystems and the CPU emulation.
...
//#endif
        ...
        if (settingsloaded == 0)
        {
          ...
          if (showcopyright(real_scrbitmap)) {
            goto userquit;
          }
        }
