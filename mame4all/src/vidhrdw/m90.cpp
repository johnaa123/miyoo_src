...
static void m90_drawsprites(struct osd_bitmap *bitmap)
{
  ...
  for (offs = 0x0; offs <0x200-8; offs+= 6)
  {
    ...
    y_multi=1;
    if ((m90_spriteram[offs+1]&0xe0)==0x20)
    {
      y=y-0x10;
      y_multi=2;
    }
    if ((m90_spriteram[offs+1]&0xe0)==0x40)
    {
      y=y-0x30;
      y_multi=4;
    }
    if ((m90_spriteram[offs+1]&0xe0)==0x60)
    {
      y=y-0x70;
      y_multi=8;
    }
...
void m90_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
  ...
  if (m90_video_control_data[0xa]&0x10) pf1_enable=0;
  else pf1_enable=1;
  if (m90_video_control_data[0xc]&0x10) pf2_enable=0;
  else pf2_enable=1;
