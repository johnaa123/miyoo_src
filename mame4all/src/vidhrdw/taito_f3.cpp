...
static void f3_tilemap_draw(struct osd_bitmap *bitmap,struct tilemap *tilemap,UINT32 trans,int pos,int sx,int sy, int dpri)
{
  ...
  if (flipscreen)
  {
    ...
    if (f3_game_config->extend)	sx=-sx+188-512;
    else sx=-sx+188; /* Adjust for flipped scroll position */
    y=255;
    if (Machine->orientation == ROT0) rot=ORIENTATION_FLIP_Y;
    else rot=ORIENTATION_FLIP_X;
  }
  else
  {
...
static void f3_fix_transparent_colours(int p0, int p1, int p2, int p3)
{
  ...
  for (i=0; i<0x10; i++)
  {
    if (p0==i)
    {
      if (f3_game_config->extend) pos=0x0000;
      else pos=0x0000;
      i=0x10;
    }
    if (p1==i)
    {
      if (f3_game_config->extend) pos=0x0800;
      else pos=0x0400;
      i=0x10;
    }
    if (p2==i)
    {
      if (f3_game_config->extend) pos=0x1000;
      else pos=0x0800;
      i=0x10;
    }
    if (p3==i)
    {
      if (f3_game_config->extend) pos=0x1800;
      else pos=0x0c00;
      i=0x10;
    }
  }
...
static void f3_drawsprites(struct osd_bitmap *bitmap)
{
  ...
  for (offs = 0; offs < sprite_top; offs += 4)
  {
      ...
      if (f3_game==DARIUSG || f3_game==GEKIRIDO || f3_game==CLEOPATR)
      {
        ...
        if ((spritecont & 0x40) == 0)
        {
          ...
          else
          {
            ...
            if (this_x&0x800) this_x= 0 - (0x800 - (this_x&0x7ff));
            else this_x&=0x7ff;
        ...
        if ((spritecont & 0x10) == 0)
        {
          ...
          else
          {
            ...
            if (this_y&0x800) this_y= 0 - (0x800 - (this_y&0x7ff));
            else this_y&=0x7ff;
    ...
    last_x=x;
    last_y=y;
    if (block_zoom_x) x_addition=(0x110 - block_zoom_x) / 16;
    else x_addition=16;
    if (block_zoom_y) y_addition=(0x110 - block_zoom_y) / 16;
    else y_addition=16;
    if (!sprite) continue;
    ...
    if (block_zoom_x) sprite_ptr->zoomx = ((0x10f-block_zoom_x)<<8)|0x80;
    else sprite_ptr->zoomx=0x10000;
    if (block_zoom_y) sprite_ptr->zoomy = ((0x10f-block_zoom_y)<<8)|0x80;
    else sprite_ptr->zoomy=0x10000;
...
void f3_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
  ...
  /* Update tilemaps */
  if (f3_game_config->extend) inc=0x800;
  else inc=0x400;
  gfx_base=f3_pf_data;
  ...
  */
  ...
  for (pos=0; pos<4; pos++)
  {
    ...
    if (flipscreen)
    {
      t_pos=0xb1fe + (pos*0x200);
      z_pos=0x81fe + (pos*0x200);
    }
    else
    {
      t_pos=0xb000+(pos*0x200);
      z_pos=0x8000+(pos*0x200);
    }
    for (i=0; i<256; i++)
    {
      ...
      {
        i=clip_pri_kludge;
        t_pos+=clip_pri_kludge;
        z_pos+=clip_pri_kludge;
      }
