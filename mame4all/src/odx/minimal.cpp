...
unsigned int odx_joystick_read()
{
  ...
  if(keystates[SDLK_UP] == SDL_PRESSED)         res|= OD_UP;
  if(keystates[SDLK_DOWN] == SDL_PRESSED)       res|= OD_DOWN;
  if(keystates[SDLK_LEFT] == SDL_PRESSED)       res|= OD_LEFT;
  if(keystates[SDLK_RIGHT] == SDL_PRESSED)      res|= OD_RIGHT;
  if(keystates[SDLK_RCTRL] == SDL_PRESSED)      res|= OD_MENU;
  if(keystates[SDLK_F10] == SDL_PRESSED)      res|= OD_MENU;
  if(keystates[SDLK_LCTRL] == SDL_PRESSED)      res|= OD_B;
  if(keystates[SDLK_LALT] == SDL_PRESSED)       res|= OD_A;
  if(keystates[SDLK_SPACE] == SDL_PRESSED)      res|= OD_Y;
  if(keystates[SDLK_LSHIFT] == SDL_PRESSED)     res|= OD_X;
  if(keystates[SDLK_BACKSPACE] == SDL_PRESSED)  res|= OD_R;
  if(keystates[SDLK_TAB] == SDL_PRESSED)        res|= OD_L;
  if(keystates[SDLK_RETURN] == SDL_PRESSED)     res|= OD_START;
  if(keystates[SDLK_ESCAPE] == SDL_PRESSED)     res|= OD_SELECT;
...
static void odx_text_log(char *texto)
{
  ...
  odx_text(od_screen16,0,log,texto,255);
  odx_video_flip();
  odx_text(od_screen16,0,log,texto,255);
  odx_video_flip(); // do twice to avoid flickering
  log+=16;
