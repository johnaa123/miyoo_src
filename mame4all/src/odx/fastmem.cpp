...
void* fast_memcpy(void *OUT, const void *IN, size_t N)
{
  ...
  else
  {
    ...
    if ((((unsigned)s)&3) != (((unsigned)d)&3))
    {
      ...
      while (n--)
      {
        dsq[0]=(s[3]<<24)|(s[2]<<16)|(s[1]<<8)|s[0];
        s+=4;
        dsq[1]=(s[3]<<24)|(s[2]<<16)|(s[1]<<8)|s[0];
        s+=4;
        dsq[2]=(s[3]<<24)|(s[2]<<16)|(s[1]<<8)|s[0];
        s+=4;
        dsq[3]=(s[3]<<24)|(s[2]<<16)|(s[1]<<8)|s[0];
        s+=4;
        dsq[4]=(s[3]<<24)|(s[2]<<16)|(s[1]<<8)|s[0];
        s+=4;
        dsq[5]=(s[3]<<24)|(s[2]<<16)|(s[1]<<8)|s[0];
        s+=4;
        dsq[6]=(s[3]<<24)|(s[2]<<16)|(s[1]<<8)|s[0];
        s+=4;
        dsq[7]=(s[3]<<24)|(s[2]<<16)|(s[1]<<8)|s[0];
        s+=4;
        d+=32;
        dsq+=8;
      }
