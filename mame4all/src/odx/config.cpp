...
void parse_cmdline (int argc, char **argv, int game_index)
{
  ...
  sprintf(dir1,"%s/nvram",mdir);
  nvdir      = get_string ("directory", "nvram",   NULL, dir1);
  sprintf(dir2,"%s/hi",mdir);
  hidir      = get_string ("directory", "hi",      NULL, dir2);
  sprintf(dir3,"%s/cfg",mdir);
  cfgdir     = get_string ("directory", "cfg",     NULL, dir3);
  sprintf(dir4,"%s/snap",mdir);
  screenshotdir = get_string ("directory", "snap",     NULL, dir4);
  sprintf(dir5,"%s/memcard",mdir);
  memcarddir = get_string ("directory", "memcard", NULL, dir5);
  sprintf(dir6,"%s/sta",mdir);
  stadir     = get_string ("directory", "sta",     NULL, dir6);
  sprintf(dir7,"%s/artwork",mdir);
  artworkdir = get_string ("directory", "artwork", NULL, dir7);
